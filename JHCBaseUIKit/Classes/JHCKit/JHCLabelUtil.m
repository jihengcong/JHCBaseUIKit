...
+ (JHCLabelUtil *)createLabelWithFrame:(CGRect)frame labelText:(NSString * _Nullable)labelText textColor:(UIColor * _Nullable)textColor textFont:(UIFont * _Nullable)textFont textAligment:(NSTextAlignment)textAligment headerImage:(UIImage * _Nullable)headerImage
{
    ...
    if (headerImage && labelText.length > 0)
    {
        NSTextAttachment *attach = [[NSTextAttachment alloc] init];
        attach.image = headerImage;
        attach.bounds = CGRectMake(0, -1, headerImage.size.width, headerImage.size.height);
...
- (void)addHeaderImage:(UIImage * _Nullable)headerImage
{
    if (headerImage && self.text.length > 0)
    {
        [self addHeaderImage:headerImage offsetY:-1];
    }
}

- (void)addHeaderImage:(UIImage * _Nullable)headerImage offsetY:(CGFloat)y
{
    if (headerImage && self.text.length > 0)
    {
        NSTextAttachment *attach = [[NSTextAttachment alloc] init];
        attach.image = headerImage;
        attach.bounds = CGRectMake(0, y, headerImage.size.width, headerImage.size.height);
